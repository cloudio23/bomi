...
// origin은 {text: "장소명"} 또는 {lat, lng}(GPS로 이미 확보된 좌표) 둘 중 하나로 옵니다.
export async function resolvePlace(origin) {
  if (origin.lat !== undefined && origin.lng !== undefined) {
    return { lat: origin.lat, lng: origin.lng, placeName: '현재 위치', address: '' };
  }
  if (origin.text) return geocodePlace(origin.text);
  return null;
}
